...
pages remain read-only; changing context capacity requires confirmation.
...
Search an action, press i to edit its JSON parameters, then Esc, Normal Enter to
review, and Enter to confirm. Parameters lock between those steps. Read-only actions send once
without a confirmation step. **Field help** shows the same schema's allowed
...
save text. In a new-session form, select the final “Review and create session”
row and press Enter, then Enter on the confirmation page.
...
For async questions, select with `j/k`, arrows, or `Ctrl+j/k`, then confirm with
