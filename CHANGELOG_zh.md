...
从本机 Claude CLI 自动发现模型，并优化行内代码高亮。通信协议保持 v72。
详见[双语发布说明](docs/releases/v4.0.4.md)。

- Web、侧聊和 TUI 的模型列表读取原生目录及支持的思考强度，不发送提示词或恢复会话。
- 打开模型设置时刷新目录，按账号、目录和 CLI 构建缓存；保留用户明确选择的历史模型。
- Work 模型发现遵循其实际运行策略；Claude 新会话继承原生模型和思考强度默认值，
  不再强制使用固定的 Opus/max 组合。
- 刷新页面、重连或打开模型菜单时重新读取 Codex 模型；同账号并发请求合并，读取
  失败时保留该账号上次成功的列表。
- 兼容 Codex 0.156 的受保护套接字别名，继续校验账号归属并检测底层监听器替换。
- 已验证的 Python Claude Agent SDK 从 0.2.151 升级至 0.2.157；跨 SDK 版本升级
  Wrapper 时，需按文档等待任务结束、迁移服务并使用安装器升级。
- 行内代码改为更柔和的引擎主题色文字，去掉背景块；真实链接保留链接样式，代码块
  仍保留背景。
- Claude 思考文本默认展开；未聚焦编辑器时按 `/` 只聚焦输入框，再按一次才输入字符。

## v4.0.3

修复 Claude 历史翻页时对话重复、后续问题被放到开头的问题。通信协议保持 v72。
详见[双语发布说明](docs/releases/v4.0.3.md)。

- 根据唯一的原生用户消息别名合并浏览器实时记录与历史分页。
- 局部更新缓存时保留消息原来的顺序，即使时间戳缺失或相同。
- 保留已展开的处理过程及原生详情查询标识，不改动实时完成状态；不同原生消息
  即使文字相同也不会被误合并。
- 读取缓存页面时修复重复副本，补充翻页、重新加载及 Chromium／iPhone WebKit 回归覆盖。
...
从 v4.0.0 首次升级的方法见[双语发布说明](docs/releases/v4.0.1.md)。

- 新增 `cc-remote update`，用于安装器管理的本机 Relay/Wrapper：发现最新稳定版、
  只查更新、指定版本、校验安装包、保留服务身份、避免并发激活，并拦截未协调的协议
  升级。复用不可变安装与回滚，独立 Claude 服务和自定义部署保留原有生命周期。
- macOS Wrapper 升级时保留用户原有环境配置。
- 按账号准备 Codex 共享 daemon，保留正在运行的原生客户端，检查 CLI 与 Wrapper
  的实际连接并在安装后显示结果。默认共享连接不可用时明确报错；保留用户关闭共享的
  设置，旧的独立终端任务结束后再重新打开。

## v4.0.0

cc-remote v4 为 Claude/Codex 控制面带来独立 Claude 会话服务、原生引导、可恢复
历史与每轮差异归档、更完整的侧聊和文件预览，以及终端工作台预览版。
功能重点与升级要求见[双语发布说明](docs/releases/v4.0.0.md)。

### 稳定性收敛

- 正确消费 Claude 后台注入响应，不再阻塞主轮结束或下一条消息；按真实原生续跑
  更新运行状态，区分子任务状态，并支持 CLI 省略内部用户回显的情况。
- 子任务在主轮执行中返回时沿用原时间线；从原生 JSONL 恢复已消费的排队提示词，
  移除没有实际过程内容的续跑折叠项。
- 首条大图片遮住 SDK 目录信息时，通过有界原生记录读取恢复原始工作目录。
- 按原生压缩边界回执去重手动压缩记录。
- 流式输出时尊重触摸、滚动条和键盘视口变化，避免抢回底部或反复抖动；拦截 iOS
  键盘重复投递的粘贴事件，同时保留用户主动连续粘贴。
- 最终协同协议为 v72，Relay、Wrapper、Web 和 TUI 必须使用一致版本。
...
- 修复引导后回答停止更新、手动展开的过程反复收起；助手询问支持 Enter 发送、
  Shift+Enter 换行、Escape 关闭。上下文读取暂时失败时保留有效读数，使用紧凑
  自定义引擎菜单，并允许失败的 Skills 请求重新加载。
- 重整中英文文档，以源码部署为主线，补充 macOS/Linux 的 Codex App、CLI 与
  Wrapper 共享 daemon 接入流程。

- 修复 Codex 上下文进度与自动压缩口径不一致（protocol v62）：按账号、会话及
  最新用量记录匹配有界本机日志中的原生估算；压缩后丢弃旧值。估算不可用时仅
  显示最近请求用量，不显示误导性百分比；运行中的可见会话自动刷新。
