...
- 将 DSH 整合到终端工作台版本的 master，保留 Web 控件、附件导入限制及
  原生会话恢复。protocol v72 合并 DSH 消息、队列排序与替换增量；TUI 重放
  使用恢复快照替换原文，避免重复追加。
...
- 在处理中的小火花旁以低调灰色直接显示原生输入/输出 token 计数，点击查看完整数字
  和缓存用量；绑定当前轮次，重连恢复不重复累计（protocol v69）。


- Claude SDK 会话明确请求思考摘要，返回内容显示在现有“思考”区域，保留原生
  思考模式、token 预算和强度。重新接回常驻会话时，通过有时限的原生控制更新
  显示方式，不重启进程、不重复提交任务；旧控制不支持时仍可恢复正在运行的会话。
  运行中的原生任务保留旧显示配置，下一轮任务才应用摘要设置，中途引导不会提前生效。

- Claude 后台任务改为输入区工具栏中的紧凑胶囊；桌面向上展开详情，手机居中显示。
  按原生任务状态移除已结束的任务，全部结束时自动关闭小窗并隐藏入口；助手回复
  完成不会提前隐藏仍在运行的后台任务。
- Claude 接入原生引导，支持常驻服务与 BTW 会话，在运行中补充输入时不再打断
  当前任务；跨轮次及重连时保留消息归属和待处理输入。
- 启动 Claude BTW 前预留私有会话身份，切换引擎时重试暂时繁忙的会话列表读取。
  修复多账号会话控制的持久化归属，隐藏尚无可读历史的 Codex 临时线程通知。
- 历史中隐藏 Claude 内部压缩命令消息，保留已完成回答的时间戳；分别显示生效的
  自动压缩阈值和模型容量。
- 定时消息增加来源标签；待发送任务的会话卡片显示圆角流动光圈及下次发送时间。
  按账号和原生队列回执关联，与 Goal 分开（protocol v68）；会话菜单和任务小窗
  自动避让侧栏底栏及手机可视区域。

- 修复 DSH 子代理历史读取和主会话列表过滤，消除首个任务的虚假历史分页；
  子代理结果唤醒父会话时，以圆角标记衔接原任务。实时与历史展示统一使用
  **protocol v67**，Web、Relay、Wrapper 需同步部署。

- 重做圆角 Goal 小窗，分别展示 Codex Token 预算、Claude 完成检查和 DSH 原生轮次上限；
  操作等待原生确认，失败或版本过期时保留输入（protocol v65）。

- 接入 DSH 原生归档、全文搜索、引用、子代理控制、后台任务、计划确认、
  产出文件、会话 ZIP 导出和连接诊断（protocol v64）。
- 目录链接关联文件浏览器；XLSX 支持工作表预览、已保存的单元格值与原文件下载，
  适用于 macOS、Linux 和手机浏览器。
- 新增可选的 DeepSeek Harness 0.1.5-rc.2 Code 引擎（protocol v61）：原生 Agent
  Preset、模型与推理强度、权限预设、目标、命令、提问与审批、附件、引导、排队、
  停止和分支。配对信息只留在设备；浏览冷历史不启动 Agent，重试、引导和重连
  按原生消息身份归属。适配桌面与手机的明暗主题，并提供隔离的原生无模型费用验收。
...
- 修复 Codex 压缩过程中发送引导后的过程归属与空详情误判，刷新或重新打开会话后仍能正确展开。
