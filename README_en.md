...
**Current release: v3.0.0** · Wire protocol v59
...
In Codex Code, use `/autocompact 200k` or the context usage popover to set a
session-only compaction threshold; `/autocompact default` restores native defaults.
Limits come from the account's native model catalog and retain its context reserve.
A threshold above the default window expands only this session, within the model limit.
...
| **Rollback-safe releases** | The product version is v3.0.0 and the wire protocol is v59. Builds and deployments validate both values. The VPS uses immutable releases, release-local virtual environments, an atomic `current` switch, and rollback instead of overwriting a live directory. |
...
> **Upgrading to protocol v59:** the wire gate rejects mixed versions. Deploy
...
> then start the v59 relay and v59 wrapper so the old wrapper cannot occupy the
...
release's `/healthz` is verified. Start the v59 wrapper after success.
