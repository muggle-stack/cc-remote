...
**Current release: v3.0.0** · Wire protocol v62
...
| **Rollback-safe releases** | The product version is v3.0.0 and the wire protocol is v62. Builds and deployments validate both values. The VPS uses immutable releases, release-local virtual environments, an atomic `current` switch, and rollback instead of overwriting a live directory. |
...
> **Upgrading to protocol v62:** the wire gate rejects mixed versions. Deploy
...
> then start the v62 relay and v62 wrapper so the old wrapper cannot occupy the
...
release's `/healthz` is verified. Start the v62 wrapper after success.
