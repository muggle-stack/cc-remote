...
**Current release: v3.0.0** · Wire protocol v61
...
the same mobile/desktop conversation and file preview interface. See
[local DSH setup and feature boundaries](integrations/dsh/README.md).
...
In Codex Code, use `/autocompact 300k` or the context usage popover to set the
...
Displayed usage is the last model response's count, separate from native compaction
accounting. `/autocompact default` restores native defaults. Limits come from the
account's native model catalog. Lowering the setting also reduces this session's window.
Busy sessions retain a pending preference until the thread is idle and other clients
...
| **Rollback-safe releases** | The product version is v3.0.0 and the wire protocol is v61. Builds and deployments validate both values. The VPS uses immutable releases, release-local virtual environments, an atomic `current` switch, and rollback instead of overwriting a live directory. |
...
Optional [remote Viewer previews](docs/remote-viewer.md) display
multi-file static interactive pages in a side panel or full-screen on mobile.
Default Bridge mode reuses the main origin, including explicitly allowed HTTP/IP access;
explicitly referenced home-directory pages are verified and associated with the
session without per-project registration. Static-server links require local
socket/process ownership proof. Extra preview domains are not required.
...
- **Concurrent Claude accounts:** `CC_REMOTE_CLAUDE_PROFILES_JSON` or
  ...
  injects only the selected directory into that Claude child. Explicit
  multi-account mode loads only the selected profile's user settings, so project
  or local settings cannot replace its provider. The wrapper never reads,
  copies, or promotes credentials through the command line, mutates its
  process-wide environment, or changes another resident session. Claude still
  discovers project instruction files natively; provider/env/helper settings
  belong in the selected profile. Empty configuration
...
- WAV, MP3, M4A/M4B, AAC, FLAC, Ogg/Opus, and WebM audio can be played, sought,
  speed-adjusted, or downloaded in the desktop and mobile file panel. Original
  encoding is preserved, with an 8 MiB file limit and a clear error for unsupported
  browser codecs. Closing, refreshing, or switching files stops playback; audio
  bytes do not enter chat history or replay.
...
> **Upgrading to protocol v61:** the wire gate rejects mixed versions. Deploy
...
> then start the v61 relay and v61 wrapper so the old wrapper cannot occupy the
...
release's `/healthz` is verified. Start the v61 wrapper after success.
