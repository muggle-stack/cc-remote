...
from cc_remote.tui_settings import SettingsForm, ValuePicker, TextValue, Confirm
...
@pytest.mark.asyncio
async def test_model_change_uses_catalog_and_explicit_confirmation():
    app, c = setup()
    async with app.run_test() as pilot:
...
async def test_new_session_collects_cwd_model_permissions_atomically(
    ...
    async with app.run_test(size=(80, 28)) as pilot:
        ...
        assert isinstance(app.screen, Confirm) and not sent(c, "new_session")
        await pilot.press("enter")
...
async def test_settings_apply_one_scoped_command_after_confirmation(
    field,
    command,
    attribute,
    value,
    ...
    async with app.run_test() as pilot:
        ...
        assert isinstance(app.screen, Confirm) and not sent(c, command)
        await pilot.press("enter")
