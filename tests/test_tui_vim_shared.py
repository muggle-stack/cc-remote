...
async def test_identical_vim_objects_counts_and_motions(surface):
    ...
    async with app.run_test() as pilot:
        ...
        for text, cursor, keys, expected in CASES:
            ...
            if surface in {"chat", "report"} and not keys.startswith("v"):
                assert (
                    widget.cursor_location
                    == widget.document.get_location_from_index(cursor)
                )
...
@pytest.mark.asyncio
async def test_yank_in_chat_replaces_old_draft_register_without_moving_reader():
    app = make_app()
    async with app.run_test() as pilot:
        reader = app.query_one(Transcript)
        editor = app.query_one(Composer)
