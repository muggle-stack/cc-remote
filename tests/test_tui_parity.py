...
@pytest.mark.asyncio
async def test_mutating_action_needs_immutable_confirmation_and_validates():
    app, client = app_client()
    async with app.run_test() as pilot:
        ...
        editor.load_text(
            '{"session_id":"s","title":"renamed","engine":"codex"}'
        )
        await pilot.press("enter")
        await pilot.pause()
        assert not client._outbox and editor.read_only
        await pilot.press("enter")
        await pilot.pause()
...
async def test_modal_actions_are_reachable_and_confirmable_without_mouse():
    ...
    async with app.run_test(size=(60, 22)) as pilot:
        ...
        await pilot.pause()
        editor = app.screen.query_one(TextArea)
        editor.load_text('{"effort":"high"}')
        editor.focus()
        await pilot.press("enter")
        assert not c._outbox and editor.read_only
        await pilot.press("enter")
