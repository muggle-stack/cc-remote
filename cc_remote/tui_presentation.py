    ...
    def panel(self, name: str) -> str:
        ...
        if name == "Usage / Context":
            parts = [
                self.usage_label(),
                "\nContext",
                describe(self.context or self.status.get("context")),
                "\nCodex context settings",
                describe(self.settings.get("codex_context")),
                "\nQuota (used_percent is consumed)",
                describe(self.rates),
            ]
        ...
        if name == "Settings":
            return (
                describe(self.settings)
                + "\n\nControl\n"
                + describe(self.control)
                + "\n\nAvailable models\n"
                + describe(self.reports.get("models"))
                + "\n\nPermission profiles\n"
                + describe(self.reports.get("permission_profiles"))
            )
        if name == "Background":
            return describe(self.background)
        if name == "Notices":
            return describe(list(self.notices.values()))
        if name == "Questions":
            return describe(list(self.questions.values()))
        return describe(self.status if name == "Status" else self.reports)
