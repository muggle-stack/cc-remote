...
class Transcript(VimArea):
    ...
    def operate(self, operator, start, end, *, linewise=False):
        if operator != "y":
            return super().operate(operator, start, end, linewise=linewise)
        position = self.document.get_index_from_location
        self.register = self.projection.extract(
        ...
        self.move_cursor(self.vim_cursor_location())
...
class WorkspaceApp(App, inherit_bindings=False):
    ...
    def action_preview_files(self) -> None:
        ...
        if len(refs) == 1:
            ...
            self.push_screen(FileHints(refs), chosen)
        else:
            self.client.notice = (
