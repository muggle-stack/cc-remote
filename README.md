...
压缩估算；后台刷新失败时保留同一会话、同一模型的最近有效读数，不显示读取提示。
尚无原生估算时，仅显示最近请求用量，不显示百分比。运行中的可见会话
...
返回目录会保留列表。Code 可直接预览运行用户有读取权限的文件，不要求文件属于该用户，
也不因文件在会话目录外再次确认；外部文件默认只读。目录按需分页，不递归扫描；
符号链接和特殊文件不打开。
