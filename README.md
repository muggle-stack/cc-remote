...
Codex Code 可用 `/autocompact 200k` 或上下文用量面板设置当前会话的自动压缩阈值，
`/autocompact default` 恢复 Codex 默认行为。上限读取当前账号的原生模型目录，并保留
模型的上下文预留空间；超过默认窗口时，仅扩大该会话的窗口，不超过模型上限。
运行中可保存，空闲且其他客户端不再保留该会话时生效；界面会区分待应用与已应用。
暂不支持 Work 和临时侧聊。切换到容量更小的模型前，须先恢复默认压缩设置。

从“更多设置 → 会话文件”或输入 `/open [路径]` 打开目录浏览。Code 从当前会话目录开始，
支持上级目录、绝对路径和 `~`；Work 保持在工作目录内。点击文件在侧栏预览，
返回目录会保留列表；会话目录外的文件沿用单文件预览授权。目录按需分页，不递归扫描；
符号链接和特殊文件不打开。
...
| **可回滚发布** | 产品版本统一为 v3.0.0，wire protocol 为 v59。构建和部署同时校验产品版本与协议版本；VPS 使用不可变 release、独立 venv、原子 `current` 切换和失败回滚，避免直接覆盖正在运行的目录。 |
...
可用状态，不会收到 `CODEX_HOME` 路径或凭据。修改后需重启 wrapper；protocol v59
...
- WAV、MP3、M4A/M4B、AAC、FLAC、Ogg/Opus、WebM 音频可在桌面和手机文件面板
  直接试听、拖动进度、调整速度或下载。保留原文件编码，单文件上限 8 MiB；浏览器不支持
  该编码时提供明确提示。关闭、刷新或切换文件会停止播放，音频不会进入聊天历史或回放。
...
> **升级到协议 v59**：线协议会严格拒绝版本不一致。请在同一次维护窗口部署
...
> 手工发布时先停本机 wrapper，再停服更新 relay + web，最后启动 v59 relay 和
> v59 wrapper；这样旧 wrapper 不会占住同一 `machine_id` 的连接槽。若从 v34 以前的
...
恢复，并验证旧 release 的 `/healthz`。成功后再启动 v59 wrapper。
