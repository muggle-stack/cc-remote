...
Protocol v69 is a coordinated upgrade: publish freshly built Relay/Web and
...
activate the v69 relay/web release; then start the v69 wrapper.
...
verify v69, and retain that snapshot with the previous release. To roll back,
stop v69, run `work_registry_snapshot.py restore`, then switch and start the old
...
Protocol v69 carries explicit DSH continuation markers in live bindings and
history summaries. It retains typed, revision-checked Goal controls, native
session tools, directory-link browsing, portable XLSX previews and Codex
continuity fixes. All three cc-remote tiers must
