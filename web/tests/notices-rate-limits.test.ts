...
try {
  ...
  assert.equal(presentCommandProblem({ code: "dsh_auth_expired", message: hiddenDiagnostic }),
    "DSH 本机配对不可用，请重新配对。");
  assert.equal(presentCommandProblem({ code: "dsh_unsupported", message: hiddenDiagnostic }),
    "DSH 暂不支持此操作。");
  assert.equal(presentCommandProblem({ code: "dsh_gateway/not-found", message: hiddenDiagnostic }),
    "DSH 接口不兼容，请检查 DSH 版本。");
