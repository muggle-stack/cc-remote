...
export function FileBrowserPanel({ sid, initialPath, ws, hidden, onListen,
  ...
  const pending = useRef<{ id: string; append: boolean; target: string } | null>(null);
  ...
  const browse = useCallback((target: string, includeHidden: boolean,
    ...
    pending.current = { id, append: offset > 0, target };
    setError(null);
    const fail = (message: string) => {
      pending.current = null;
      setLoading(false);
      setError(message);
      if (!offset) { setListing(null); setPath(target); }
    };
    ...
    setLoading(true);
    timeout.current = setTimeout(() => fail("读取目录超时，请重试"), 15000);
  }, [ws, sid]);
  useEffect(() => {
    onListen((message) => {
      if (message.sid !== sid || message.request_id !== pending.current?.id) return;
      const { append, target } = pending.current;
      pending.current = null;
      if (timeout.current) clearTimeout(timeout.current);
      setLoading(false);
      if (message.error) {
        setError(message.error);
        if (!append) { setListing(null); setPath(target); }
        return;
      }
  ...
  const visibleListing = loading && !pending.current?.append ? null : listing;
  return <div className="workspace-files" hidden={hidden} data-lock-horizontal-swipe="true">
    ...
    <div className="artifact-head">
      ...
      <span className="artifact-path" title={visibleListing?.path}>{visibleListing?.path}</span>
      <button className="iconbtn" title="刷新目录" aria-label="刷新目录"
        onClick={() => browse(listing?.path ?? path, showHidden)}><Icon name="refresh" size={17} /></button>
    ...
    <div className="workspace-options">
      ...
      <label><input type="checkbox" checked={showHidden} onChange={(event) => {
        ...
        browse(listing?.path ?? path, event.target.checked);
      }} />隐藏文件</label>
    </div>
    {error && <p className="workspace-error" role="alert">{error}</p>}
    <div className="workspace-entries" aria-busy={loading}>
      {visibleListing?.entries.map((entry) => <button key={entry.path} className="workspace-entry"
