import { MAX_ATTACHMENT_COUNT, MAX_IMAGE_SOURCE_BYTES } from "./img";
import type { ClipboardImport } from "./clipboard-import";
...
export async function resolveClipboardFiles(snapshot: ClipboardImport): Promise<{
  ...
  let unavailable = 0;
  let native = false;
  for (const [index, source] of snapshot.images.entries()) {
    try {
  ...
  unavailable = Math.max(0, unavailable
    - snapshot.files.filter((file) => IMAGE_TYPES.has(file.type)).length);
  ...
  for (const [index, file] of files.entries()) {
    ...
    const supplied = index < snapshot.files.length;
    ...
    if (unique.length >= MAX_ATTACHMENT_COUNT) {
      errors.push(`一次最多导入 ${MAX_ATTACHMENT_COUNT} 个附件。`);
      break;
    }
