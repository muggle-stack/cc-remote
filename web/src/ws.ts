...
export class RelayWs {
  ...
  private sendSessionListRead(
    engine: "claude" | "codex" | "dsh", space: Space, retry = false,
  ): string | null {
  ...
  private sendListRefreshingCommand(
    ...
    this.pendingListRequests.set(commandId, {
      ownership,
    ...
    if (this.pendingListRequests.size > OUTBOX_MAX_COMMANDS) {
      this.pendingListRequests.delete(this.pendingListRequests.keys().next().value!);
    }
    const queued = this.sendTracked(obj, commandId);
    if (!queued) this.pendingListRequests.delete(commandId);
    return queued;
  }
      ...
      try {
        ...
        if (msg.type === "session_list") {
          ...
          const listedRequest = msg.request_id
            ? this.pendingListRequests.get(msg.request_id)
            : undefined;
          const acceptedOrder =
        ...
        if (msg.type === "error" && msg.request_id) {
          const listedRequest = this.pendingListRequests.get(
            msg.request_id);
